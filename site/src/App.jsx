...
const ECOSYSTEMS = [
  ...
  },
  {
    ...
    rows: [
      ...
      },
      {
        ...
        detect: (
          <>
            <code>pixi.toml</code>, <code>pixi.lock</code>,{" "}
            <code>[tool.pixi]</code>
          </>
        ),
        deletes: <code>.pixi</code>,
        verify: (
          <>
            <code>pixi.lock</code> carries its <code>version:</code> and
            records at least one conda or PyPI package
          </>
        ),
        restore: <code>pixi install</code>,
      },
    ],
    tieBreak: (
      <>
  ...
  {
    id: "eco-jvm",
    language: "JVM, Swift, Dart, C/C++, Zig, Haskell & game engines — opt-in",
    summary:
    ...
    rows: [
      ...
      {
        name: "zig",
        detect: <code>build.zig</code>,
        deletes: (
          <>
            <code>.zig-cache</code>, <code>zig-cache</code>, <code>zig-out</code>
          </>
        ),
        verify: (
          <>
            <code>build.zig</code> declares a <code>pub fn build</code>
          </>
        ),
        restore: (
          <>
            next <code>zig build</code>
          </>
        ),
      },
      {
        name: "stack",
        detect: <code>stack.yaml</code>,
        deletes: <code>.stack-work</code>,
        verify: (
          <>
            <code>stack.yaml</code> names its <code>resolver:</code> (or{" "}
            <code>snapshot:</code>), so the rebuild compiles against the same
            pinned snapshot
          </>
        ),
        restore: (
          <>
            next <code>stack build</code>
          </>
        ),
      },
      {
        name: "cabal",
        detect: <code>cabal.project</code>,
        deletes: <code>dist-newstyle</code>,
        verify: (
          <>
            <code>cabal.project</code> declares its <code>packages</code>; a
            lone <code>*.cabal</code> file is never claimed
          </>
        ),
        restore: (
          <>
            next <code>cabal build</code>
          </>
        ),
      },
      {
        name: "sbt",
        detect: <code>build.sbt</code>,
        deletes: (
          <>
            <code>target</code>, <code>project/target</code>
          </>
        ),
        verify: (
          <>
            <code>build.sbt</code> carries a setting, or{" "}
            <code>project/build.properties</code> pins <code>sbt.version</code>
          </>
        ),
        restore: (
          <>
            next <code>sbt compile</code>
          </>
        ),
      },
    ],
    tieBreak: (
      <>
        ...
        not a download — so these seventeen, and Cargo above, ship disabled and
        ...
        <code>enable_defold true</code> / <code>enable_cocos true</code> /{" "}
        <code>enable_zig true</code> / <code>enable_stack true</code> /{" "}
        <code>enable_cabal true</code> /{" "}
        <code>enable_sbt true</code> turns them on, and their candidates wait
...
export default function App() {
  ...
  const installCommands = {
    ...
    npm: {
      ...
      note: "One small package that pulls in the single platform binary matching your machine — no postinstall download, so it works under npm ci --ignore-scripts and behind a registry mirror. Swap in npx dev-prune status to run it once without installing. Windows needs 1.8.0 or later. bun add -g dev-prune@latest, pnpm add -g dev-prune@latest and yarn global add dev-prune@latest install the same package, and dev-prune treats each as its own channel: a copy bun installed is upgraded and removed with bun.",
      cmd: "npm install -g dev-prune@latest",
    },
    python: {
      group: "manager",
      label: "uv / pipx",
      registry: "PyPI — which uv, pipx and pip all read, so one upload serves the three of them.",
      note: "Platform wheels carrying the binary. Nothing Python runs. Swap in uvx dev-prune status to run it once and leave nothing behind, or pipx install --force dev-prune.",
      cmd: "uv tool install dev-prune@latest",
    },
    pip: {
      group: "manager",
      label: "pip",
      registry: "PyPI — the same wheels uv tool and pipx install.",
      note: "The same wheels, into whichever environment is active — a venv's Scripts/bin rather than a shared tool directory. Use pip install --user --upgrade dev-prune for a machine-wide install.",
      cmd: "pip install --upgrade dev-prune",
    },
    cargo: {
      group: "manager",
      label: "Cargo",
          ...
          <div className="container hero-grid">
            <div className="hero-text">
              ...
              <p className="hero-description">
                ...
                thirty-five managers from npm and pip to Composer, Bundler,
...
└── scripts/
    └── requirements.txt`}</pre>
            ...
            <div className="info-card eco-contribute">
              <h3>
                <Puzzle size={18} /> Thirty-six would be better than
                thirty-five
              </h3>
              <p>
          ...
          <div className="container narrow">
            ...
            <div className="faq-list">
              ...
              <Faq q="Does it delete build output — dist/, .next/, target/?">
                ...
                quietly. The
                eighteen exceptions are opt-in and say so —{" "}
                ...
                <code>build/</code>), <code>enable_cocos</code> (
                <code>library/</code>+<code>temp/</code>, only when the{" "}
                <code>package.json</code> names Cocos Creator),{" "}
                <code>enable_zig</code> (<code>.zig-cache/</code>+
                <code>zig-cache/</code>+<code>zig-out/</code>),{" "}
                <code>enable_stack</code> (<code>.stack-work/</code>),{" "}
                <code>enable_cabal</code> (<code>dist-newstyle/</code>) and{" "}
                <code>enable_sbt</code> (<code>target/</code>+
                <code>project/target/</code>) — whose

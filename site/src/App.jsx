...
const ECOSYSTEMS = [
  ...
  },
  {
    ...
    language: "Python",
    summary:
      "Three managers that can describe the same project, because a uv or Poetry project is still a directory with a virtual environment in it.",
    rows: [
      {
        name: "uv",
        detect: (
  ...
  {
    id: "eco-jvm",
    language: "Java & JVM — opt-in",
    summary:
      "Two build tools whose recoverability proof is the manifest itself: a build tree is regenerated by recompiling the sources sitting next to it, not by downloading.",
    rows: [
      {
        name: "Gradle",
        detect: (
    ...
    tieBreak: (
      <>
        ...
        not a download — so these two ship disabled and invisible.{" "}
        <code>devp config set enable_gradle true</code> /{" "}
        <code>enable_maven true</code> turns them on, and their candidates wait
        for <code>build_idle_days</code> (60 by default), applied as the{" "}
        <em>maximum</em> of it and <code>idle_days</code> — the build-tool gate
        can only ever make pruning later, never earlier. The dependencies
        themselves never lived in the repository: <code>~/.m2</code> and{" "}
        <code>~/.gradle</code> are machine-wide stores, which is{" "}
        <code>devp caches</code> territory.
      </>
    ),
  },
];
...
└── scripts/
    └── requirements.txt`}</pre>
            ...
            <div className="info-card eco-contribute">
              <h3>
                <Puzzle size={18} /> Twelve would be better than eleven
              </h3>
              <p>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      <code>--install</code> upgrades through the channel that
                      owns this copy
                    </td>
                  </tr>
                  <tr>

...
fn status_tag(status: &PruneStatus) -> &'static str {
    match status {
        ...
        PruneStatus::SkippedAdapterWindow(_) => "skipped_adapter_window",
        ...
        PruneStatus::NotARepo => "not_a_repo",
...
pub fn lockfile_fix_command(adapter: &str) -> Option<&'static str> {
    Some(match adapter {
        ...
        // vcpkg, cmake_build, dotnet_build and godot verify the manifest, not lockfile
        // sync — a missing manifest, or a `vcpkg.json` that declares no dependencies,
        // has no mechanical fix either.
        _ => return None,
    })
}
...
mod tests {
    ...
    fn every_status_has_a_distinct_stable_tag() {
        let all = [
            ...
            PruneStatus::SkippedAdapterWindow(45),
    ...
    fn every_adapter_with_a_lockfile_has_a_fix_command() {
        for adapter in crate::adapters::get_all_adapters() {
            // venv, gradle, maven, swift, vcpkg, cmake_build, dotnet_build and godot
            // verify without a lockfile-sync step — see `lockfile_fix_command` for why
            // each has nothing mechanical to hand over.
            if matches!(
                adapter.name(),
                "venv"
                    | "gradle"

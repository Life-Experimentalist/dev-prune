...
#### Cause
`dev-prune` enforces lockfile integrity before deleting any dependency directory — `node_modules`, `.venv`, `vendor`, `Pods`, whichever of the thirty-five adapters matched — and that check is **read-only**: it proves the lockfile can rebuild the tree, and refuses rather than repairing a lockfile that cannot. This error occurs if:

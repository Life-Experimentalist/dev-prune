...
#### Cause
`dev-prune` enforces lockfile integrity before deleting any bloat directory (`node_modules`, `.venv`, `target`, `vendor`), and that check is **read-only**: it proves the lockfile can rebuild the tree, and refuses rather than repairing a lockfile that cannot. This error occurs if:

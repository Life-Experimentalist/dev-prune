...
- **`prunable.directories`** in either file names directories no adapter can recognise, each with the `rebuild` command that puts it back. Because the committed file arrives in a clone, a declaration is checked rather than obeyed: the path must be relative with no `..`, must resolve inside the repository even through a symlinked parent, must hold no Git-tracked file, and the first word of `rebuild` must be a program this machine has. Where a manifest already in the tree can say what that program was asked to do — an `npm`/`pnpm`/`yarn` script in `package.json`, a `make` target, a `uv run` entry point in `pyproject.toml`, a `cargo` subcommand — that manifest is read too, and a declaration naming a target it does not define is refused. Every one of those checks is a read; the `rebuild` command is never run. A command shape none of them can resolve is allowed through rather than guessed at, because a false refusal blocks a prune that was safe. A declaration that fails any of those is reported and nothing is deleted. This is the only mechanism by which dev-prune deletes a directory no adapter claimed, and it has no bypass either.
- **`prunable.exclude`** in either file names declared paths to leave alone on this machine, whoever declared them: the committed file is the whole team's, and a directory the project regenerates can be the one holding something here. It is spelled the same way as a `path` and honoured from whichever file names it, because it can only ever take a directory out of the pass — a key that only deletes less needs no precedence rule to be safe.

...
1. **Download Archive**: Download `dev-prune-v1.3.1-windows-x64.zip` from GitHub Releases.
...
1. **Download Archive**:
   - Apple Silicon (M1/M2/M3/M4): `dev-prune-v1.3.1-darwin-arm64.tar.gz`
   - Intel Mac: `dev-prune-v1.3.1-darwin-x64.tar.gz`
2. **Extract & Relocate Binary**:
   ...
   mkdir -p "$BIN"
   tar -xzf dev-prune-v1.3.1-darwin-*.tar.gz -C "$BIN"
...
1. **Download Archive**: Download `dev-prune-v1.3.1-linux-x64.tar.gz` from GitHub Releases.
2. **Extract & Relocate Binary**:
   ```bash
   mkdir -p ~/.config/dev-prune/bin
   tar -xzf dev-prune-v1.3.1-linux-x64.tar.gz -C ~/.config/dev-prune/bin/

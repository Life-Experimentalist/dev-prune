...
1. **Download Archive**: Download `dev-prune-v1.5.1-windows-x64.zip` from GitHub Releases.
   On Windows on ARM take `dev-prune-v1.5.1-windows-arm64.zip` instead, and on a machine
   with no 64-bit mode at all take `dev-prune-v1.5.1-windows-x86.zip`. Everything below
...
1. **Download Archive**:
   - Apple Silicon (M1/M2/M3/M4): `dev-prune-v1.5.1-darwin-arm64.tar.gz`
   - Intel Mac: `dev-prune-v1.5.1-darwin-x64.tar.gz`
2. **Extract & Relocate Binary**:
   ...
   mkdir -p "$BIN"
   tar -xzf dev-prune-v1.5.1-darwin-*.tar.gz -C "$BIN"
...
1. **Download Archive**: Download `dev-prune-v1.5.1-linux-x64.tar.gz` from GitHub Releases.
2. **Extract & Relocate Binary**:
   ```bash
   mkdir -p ~/.config/dev-prune/bin
   tar -xzf dev-prune-v1.5.1-linux-x64.tar.gz -C ~/.config/dev-prune/bin/

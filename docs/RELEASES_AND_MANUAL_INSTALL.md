...
cargo, npm, bun, pnpm, yarn, uv, pipx, Homebrew, Scoop or WinGet copy — the install still
succeeds, still
...
1. **Download Archive**: Download `dev-prune-v1.20.0-windows-x64.zip` from GitHub Releases.
   On Windows on ARM take `dev-prune-v1.20.0-windows-arm64.zip` instead, and on a machine
   with no 64-bit mode at all take `dev-prune-v1.20.0-windows-x86.zip`. Everything below
...
3. **Extract & Copy Binary**: Extract `dev-prune.exe` into `$env:APPDATA\dev-prune\bin\dev-prune.exe`.
   ...
   ```bash
   sha256sum -c dev-prune-v1.20.0-windows-x64.zip.contents.sha256
   ```
...
1. **Download Archive**:
   - Apple Silicon (M1/M2/M3/M4): `dev-prune-v1.20.0-darwin-arm64.tar.gz`
   - Intel Mac: `dev-prune-v1.20.0-darwin-x64.tar.gz`
2. **Extract & Relocate Binary**:
   ...
   mkdir -p "$BIN"
   tar -xzf dev-prune-v1.20.0-darwin-*.tar.gz -C "$BIN"
...
1. **Download Archive**: Download `dev-prune-v1.20.0-linux-x64.tar.gz` from GitHub Releases.
2. **Extract & Relocate Binary**:
   ```bash
   mkdir -p ~/.config/dev-prune/bin
   tar -xzf dev-prune-v1.20.0-linux-x64.tar.gz -C ~/.config/dev-prune/bin/

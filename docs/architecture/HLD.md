...
- **The adapters**: Thirty-five of them, one per package manager, each enforcing lockfile safety and owning that ecosystem's directories. JavaScript (npm, pnpm, Yarn, Bun, Deno), Python (uv, Poetry, PDM, Pipenv, venv, pixi), Rust (Cargo), Go, PHP (Composer), Ruby (Bundler), Elixir (Mix, and Mix builds), Apple (CocoaPods, SwiftPM), Dart, Infrastructure (Terraform), JVM (Gradle, Maven, sbt), C/C++ (vcpkg, CMake builds), .NET (dotnet_build), Zig, Haskell (Stack, Cabal) and the game engines (Godot, Unity, Unreal, Defold, Cocos Creator). Eighteen are opt-in; the list is in [`docs/CLI_REFERENCE.md`](../CLI_REFERENCE.md).

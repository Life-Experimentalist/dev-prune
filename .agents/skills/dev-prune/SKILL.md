---
name: dev-prune
description: Use when asked to reclaim disk space, clean or prune dependency directories in any ecosystem (node_modules, .venv, target, vendor, Pods, _build, .terraform and the rest of its thirty-five package managers), restore project dependencies, size package manager caches, or diagnose the devp CLI. Covers the full dev-prune (devp) command surface, its safety rules, exit codes, and troubleshooting.
---
...
**Skill version: 1.22.0.** This file describes that release of `devp` and no other, and
...
5. **dev-prune never upgrades itself unasked.** Its one background network request is a
   release check against GitHub's public releases page (no body, no identifier, no usage
   data, at most weekly, disabled by `devp config set update_check false`;
   `DEV_PRUNE_OFFLINE=1` keeps the whole process off the network regardless of any
   setting). Nothing is downloaded until someone says yes: `devp update` asks `[y/N]` at
   a terminal when a newer release exists, and that or `devp update --install` fetches the release binary for
...
- Never touches a package-manager cache on its own. No prune pass, scheduler or Git hook
  ...
  `devp caches clear <manager>` runs it — the report prints both that and the manager's
  own command per row, because only the first is counted in `devp stats` — but only when
  the user asks for it, and it asks
...
- Never deletes container disk on its own. No prune pass, scheduler or Git hook reaches
  an engine, and none ever will. `devp caches clear <engine>` — `docker`, `podman`,
  `nerdctl`, `finch`, Apple's `container` — does, when the user names it: it runs `builder prune -a -f`, `image prune
  -a -f` and `container prune -f`, prints them first, asks, and counts what came back on
  `devp stats`. Suggest it when the user wants disk space and has containers; the cost is
  a pull of the whole layer stack and a cold build cache next time.
- **Never a bulk volume delete, and never `docker system prune --volumes`.** Not through
  dev-prune, whose table contains no `volume prune` and no `--volumes` anywhere, and not
  by you running one yourself on the user's behalf. An image can be pulled again; what is
  inside a volume is the only copy, and `--volumes` is how a local database disappears.
  If the user asks to reclaim volume space, point them at `devp caches clear docker
...
| a build fails because `node_modules` (or `.venv`, `vendor`, any pruned directory) is missing | `devp restore <path>`: anything devp removed goes back through devp, not through `npm ci`, `pip install` or the manager run by hand. Restore reuses the same detection, so a monorepo gets every project's install in one command, virtual environments come back on the Python they were built with, and the timed reinstall feeds the `restore_estimate` other commands report. When in doubt whether devp took it at all, `devp history` names the pass; `devp restore --last-run` covers everything the last pass deleted || "where is my disk actually going?" / "how big is my npm cache?" | `devp caches` — sizes every package-manager cache and store (npm, pnpm, yarn, bun, uv, pip, conda, cargo, go, maven, gradle, nuget, vcpkg, conan, ccache, sccache, composer, cocoapods, hex, bundler, dart, swift, terraform, poetry, pdm, playwright, puppeteer, huggingface, cypress, electron, deno) and prints the command that clears each. Read-only |
...
| "how much is Docker using?" / "what is taking my disk, I have containers" | `devp caches docker` (or `caches podman`, or `caches containers` for every engine plus local Kubernetes clusters) — images, containers, volumes and build cache, each sized, with how much the engine says is reclaimable. **This report deletes nothing**; `devp caches clear docker` is what deletes, when the user names the engine. Figures come from the engine's own `system df`, not a directory walk — on Docker Desktop and Podman the store is inside a VM disk the host cannot see. An engine that is installed with its daemon stopped is reported as that, with no figures: a blank, not a zero. `devp caches` shows a one-line summary per engine, outside its own total |
| "clear my docker build cache" / "reclaim the container space" | `devp caches clear docker` (or `podman`, `nerdctl`, `finch`, `container`) — runs the narrow prune commands, after printing them and asking, and counts the result on `devp stats`. `--dry-run` shows the commands and the estimate. **Touches no volume on its own**: the estimate says how much unused-volume space is being left alone, and `--include-volumes` (docker and podman) adds a pick list: unused volumes by name, the user types the numbers, one unforced `volume rm` each. That flag refuses `--yes`, `--json` and a piped stdin, so only a person at a terminal can use it; never `volume prune`, never `--volumes`. Not reachable from `clear all`, from any scheduler, or from a Git hook — naming the engine is the consent |
...
| "clear just these two caches" / "clear everything except npm" | The target takes a comma list: `devp caches clear npm,uv` empties exactly those — a one-time whitelist. The mirror is `devp caches clear all --except npm,uv` — everything but the names given, a one-time blacklist. `--except` only pairs with `all` (a named list already says what to clear), `all` inside a list is exit 2, and an engine or `maven` anywhere in a list is exit 2 — an engine's consent is naming it alone, and maven is never cleared, list or no list |
| "what is dev-prune allowed to do on my machine?" / "is this safe?" | `devp trust` — the guarantees the code enforces, then this machine read live: scheduler, hooks, and the three settings that widen anything (`require_confirmation=false`, `allow_manifest_rewrite`, opt-in adapters — `enable_cargo`, `enable_gradle`, `enable_maven`, `enable_swift`, `enable_dart`, `enable_mix_build`, `enable_vcpkg`, `enable_cmake_build`, `enable_dotnet_build`, `enable_godot`, `enable_unity`, `enable_unreal`, `enable_defold`, `enable_cocos`, `enable_zig`, `enable_stack`, `enable_cabal`, `enable_sbt`), then every copy of `dev-prune`/`devp`/`devpw` on the machine — everything on `PATH` plus each channel's install directory even when it has dropped off `PATH` — with the version it was built as, the manager that installed it, its SHA-256 and a VirusTotal lookup URL for that digest, the running one first. The version is read out of the file rather than by running it, so it is absent for anything built before 1.17.0, and the highest copy is marked `latest release` only when a release check has confirmed that; otherwise `newest here`, which is a claim about this disk and not about the project. Read-only; the digests are computed locally and the URLs are printed, never fetched, nothing is uploaded and nothing found is executed. Several copies is normal, not a finding: report the differing digests as "these update separately" and point at `devp uninstall` for the one they do not want |
...
| "fix whatever's broken" | `devp doctor --fix` — repairs installed-but-broken integrations (stale twin, stale `devpw` scheduler binary, dead-target hooks/scheduler, drifted chain, missing SKILL.md, dead registry entries); never a first-time install |
...
| "is there a newer version?" | `devp update` — prints the installed and latest versions plus the right upgrade command, then asks `[y/N]` before installing when a newer release exists and stdin is a terminal; run from a script or a pipe it never asks and installs nothing |
| "upgrade it" | `devp update --install` — downloads the release binary from GitHub, verifies its SHA-256, and replaces every copy this install runs (managed binary, `devp` alias, the running binary), then brings the `devpw` scheduler twin forward through its own verified download, a separate build the console bytes cannot become; a stale twin is reconciled even when the consoles are already current. The package manager that delivered the first copy is not run, and the one command that resyncs its version record is printed. Falls back to that manager's own upgrade command if there is no binary for this platform. `auto_update` does the download half by itself after a pass and is on by default; `devp config set auto_update false` stops it, and `devp config set version_lock true` stops every update path at once |
...
| "clean my Rust `target/` too" / "clean my Java/Gradle/Maven builds too" / "clean my Flutter build caches too" / "clean my Elixir `_build/` too" / "clean my C++ `vcpkg_installed/` too" / "clean my CMake `build/` too" / "clean my .NET `bin/` and `obj/` too" / "clean my Godot `.godot/` too" / "clean my Unity `Library/` too" / "clean my Unreal `Intermediate/` too" / "clean my Zig caches too" / "clean my Haskell `.stack-work/` or `dist-newstyle/` too" / "clean my sbt `target/` too" | `devp config set enable_cargo true` / `enable_gradle true` / `enable_maven true` / `enable_swift true` / `enable_dart true` / `enable_mix_build true` / `enable_vcpkg true` / `enable_cmake_build true` / `enable_dotnet_build true` / `enable_godot true` / `enable_unity true` / `enable_unreal true` / `enable_defold true` / `enable_cocos true` / `enable_zig true` / `enable_stack true` / `enable_cabal true` / `enable_sbt true` — opt-in adapters, idle-gated separately by `build_idle_days` (45). cmake_build claims only a tree holding a `CMakeCache.txt` that names a source directory inside the same repository, so a `build/` made by hand is never touched. dotnet_build claims `bin/`+`obj/` only when `obj/project.assets.json` records this directory’s own project file. godot claims `.godot/` (and `.import/` on Godot 3) beside a `project.godot` that carries a `config_version=` line; the editor re-imports the assets the next time it opens the project. unity claims `Library/` and `Temp/` beside a `ProjectSettings/ProjectVersion.txt`, and refuses while `Temp/UnityLockfile` says the editor is open. unreal claims `DerivedDataCache/` and `Intermediate/` beside a `.uproject`, never `Saved/`, `Binaries/` or `Content/`. defold claims `build/` beside a `game.project`. cocos claims `library/` and `temp/` only when `package.json` names Cocos Creator, so a plain Node project is never touched. zig claims `.zig-cache/`, `zig-cache/` and `zig-out/` beside a `build.zig` that declares a `pub fn build`. stack claims `.stack-work/` beside a `stack.yaml` that names its `resolver:`. cabal claims `dist-newstyle/` beside a `cabal.project`; a lone `*.cabal` file is never claimed. sbt claims `target/` and `project/target/` beside a `build.sbt` |
...
| "give my editor's agent the rules" | `devp skill --agent <editor>` — writes the per-repository rules file that editor reads. Own file: `cursor`, `windsurf`, `antigravity`, `cline`, `roo`, `kilocode`, `continue`, `amazon-q`, `kiro`, `trae`. Marked block inside a shared file: `agents-md` (`AGENTS.md` — Codex, Jules, Amp, OpenCode), `copilot`, `gemini`, `junie`, `zed`, `aider` (`CONVENTIONS.md`, which Aider reads only once `read: CONVENTIONS.md` is in `.aider.conf.yml` or `--read CONVENTIONS.md` is passed — the command says so after writing it). `devp skill --help` lists the exact paths. Not sure which editors are here? Plain `devp skill` ends with a detection report — every editor whose traces are on the machine (home-directory footprint, installed VS Code-family extension) or in the repository, whether its rules here are current, stale or missing, and the command that writes them; the report is read-only. `devp skill --detected`, from the repository root, writes rules for everything on it in one pass |
...
devp stats --json           # what has already been reclaimed, and by which repositories
                            # .lifetime.cache_bytes_freed and .container_bytes_freed are
                            # `caches clear`, counted apart and never added in
...
devp caches containers --json  # what every installed engine holds; carries no prune command
devp caches clear docker --dry-run --json  # the commands it would run, and the estimate;
...
| `summary.freed_bytes` (caches clear, engine) | `system df` before minus after — never the sum of `steps[]`. Layers are shared, so those add up to more than the disk ever held |
...
| dotnet_build *(opt-in)* | `*.csproj` / `*.fsproj` / `*.vbproj` | `bin`, `obj` | `obj/project.assets.json` parses and records this directory's own project file; `bin` only alongside that proven `obj`, and only when it holds nothing but `Debug`/`Release` directories | next `dotnet build` |
| godot *(opt-in)* | `project.godot` | `.godot` (Godot 4), `.import` (Godot 3) | `project.godot` is readable and carries a `config_version=` line | editor re-imports on next open, or `godot --headless --import` |
| unity *(opt-in)* | `ProjectSettings/ProjectVersion.txt` | `Library`, `Temp` (never `Assets`, `Packages` or `ProjectSettings`) | `ProjectVersion.txt` carries an `m_EditorVersion:` line, and `Temp/UnityLockfile` is absent (an open editor is refused, not raced) | editor re-imports on next open |
| unreal *(opt-in)* | any `*.uproject` file | `DerivedDataCache`, `Intermediate` (never `Saved`, `Binaries` or `Content`) | the `.uproject` parses as JSON with a `"FileVersion"` key | editor rebuilds the DDC on next open; regenerate project files for the IDE |
| defold *(opt-in)* | `game.project` | `build` (never `.internal` or `assets`) | `game.project` carries a `[project]` section | next build from the editor or `bob.jar` |
| cocos *(opt-in)* | `package.json` naming Cocos Creator | `library`, `temp` (never `build`, which is exported output) | `package.json` names `"creator"` or a `cocos-creator` engine, so a plain Node project is never claimed | editor re-imports on next open |
| zig *(opt-in)* | `build.zig` | `.zig-cache`, `zig-cache`, `zig-out` | `build.zig` declares a `pub fn build` | next `zig build` |
| stack *(opt-in)* | `stack.yaml` | `.stack-work` | `stack.yaml` names its `resolver:` (or `snapshot:`) | next `stack build` |
| cabal *(opt-in)* | `cabal.project` | `dist-newstyle` | `cabal.project` declares its `packages` (a lone `*.cabal` is never claimed) | next `cabal build` |
| sbt *(opt-in)* | `build.sbt` | `target`, `project/target` | `build.sbt` carries a setting, or `project/build.properties` pins `sbt.version` | next `sbt compile` |

cargo, gradle, maven, swift, dart, mix_build, vcpkg, cmake_build, dotnet_build, godot, unity, unreal, defold, cocos, zig, stack, cabal and sbt ship **disabled**
...
`enable_mix_build true` / `enable_vcpkg true` / `enable_cmake_build true` / `enable_dotnet_build true` / `enable_godot true` / `enable_unity true` / `enable_unreal true` / `enable_defold true` / `enable_cocos true` / `enable_zig true` / `enable_stack true` / `enable_cabal true` / `enable_sbt true`; while disabled
...
| `enable_dotnet_build` | `false` | Turn the opt-in .NET adapter on (`bin/` and `obj/`, claimed only when NuGet's `obj/project.assets.json` records this directory's own project file — a `bin/` holding anything of yours is never touched) |
| `enable_godot` | `false` | Turn the opt-in Godot adapter on (`.godot/`, and `.import/` on Godot 3, hold the editor's imported-resource cache; the editor rebuilds it from your assets the next time it opens the project) |
| `enable_unity` | `false` | Turn the opt-in Unity adapter on (`Library/` and `Temp/` hold the editor's import cache, claimed only beside a `ProjectSettings/ProjectVersion.txt`; refused while `Temp/UnityLockfile` says the editor is open; the editor re-imports on next open) |
| `enable_unreal` | `false` | Turn the opt-in Unreal adapter on (`DerivedDataCache/` and `Intermediate/` beside a `.uproject`; never `Saved/`, `Binaries/` or `Content/`; the editor rebuilds the DDC on next open) |
| `enable_defold` | `false` | Turn the opt-in Defold adapter on (`build/` beside a `game.project`; the next build regenerates it; `.internal/` and `assets/` are never claimed) |
| `enable_cocos` | `false` | Turn the opt-in Cocos Creator adapter on (`library/` and `temp/`, claimed only when `package.json` names Cocos Creator; exported `build/` output is never claimed) |
| `enable_zig` | `false` | Turn the opt-in Zig adapter on (`.zig-cache/`, `zig-cache/` and `zig-out/` beside a `build.zig` that declares a `pub fn build`; the next `zig build` puts all three back) |
| `enable_stack` | `false` | Turn the opt-in Haskell Stack adapter on (`.stack-work/` beside a `stack.yaml` that names its `resolver:` or `snapshot:`; the pinned snapshot makes the rebuild reproducible) |
| `enable_cabal` | `false` | Turn the opt-in Haskell Cabal adapter on (`dist-newstyle/` beside a `cabal.project` that declares its `packages`; a lone `*.cabal` file is never claimed) |
| `enable_sbt` | `false` | Turn the opt-in sbt adapter on (`target/` and `project/target/` beside a `build.sbt` that carries a setting, or whose `project/build.properties` pins `sbt.version`) |
| `build_idle_days` | `45` | Extra idle gate for the build-tool adapters (cargo, gradle, maven, swift, dart, mix_build, vcpkg, cmake_build, dotnet_build, godot, unity, unreal, defold, cocos, zig, stack, cabal, sbt); applied as `max(build_idle_days, idle_days)` |
...
`enable_mix_build`, `enable_vcpkg`, `enable_cmake_build`, `enable_dotnet_build`, `enable_godot`, `enable_unity`, `enable_unreal`, `enable_defold`, `enable_cocos`, `enable_zig`, `enable_stack`, `enable_cabal` and `enable_sbt`, all of which make build
...
| **No longer a git repository** | The directory is still on disk but its `.git` is gone — the clone was recreated by hand, or `git worktree prune` removed a worktree | `devp unlink <path>` drops the entry (`--missing` will not — it only clears paths that are gone), or `git init <path>` makes it a repository again if the directory is worth keeping tracked. Reported, never an error: the pass still exits `0` |

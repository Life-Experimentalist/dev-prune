---
name: dev-prune
description: Use when asked to reclaim disk space, clean or prune dependency directories in any ecosystem (node_modules, .venv, target, vendor, Pods, _build, .terraform and the rest of its twenty-six package managers), restore project dependencies, size package manager caches, or diagnose the devp CLI. Covers the full dev-prune (devp) command surface, its safety rules, exit codes, and troubleshooting.
---
...
| "clean my Rust `target/` too" / "clean my Java/Gradle/Maven builds too" / "clean my Flutter build caches too" / "clean my Elixir `_build/` too" / "clean my C++ `vcpkg_installed/` too" / "clean my CMake `build/` too" / "clean my .NET `bin/` and `obj/` too" / "clean my Godot `.godot/` too" | `devp config set enable_cargo true` / `enable_gradle true` / `enable_maven true` / `enable_swift true` / `enable_dart true` / `enable_mix_build true` / `enable_vcpkg true` / `enable_cmake_build true` / `enable_dotnet_build true` / `enable_godot true` — opt-in adapters, idle-gated separately by `build_idle_days` (45). cmake_build claims only a tree holding a `CMakeCache.txt` that names a source directory inside the same repository, so a `build/` made by hand is never touched. dotnet_build claims `bin/`+`obj/` only when `obj/project.assets.json` records this directory’s own project file. godot claims `.godot/` (and `.import/` on Godot 3) beside a `project.godot` that carries a `config_version=` line; the editor re-imports the assets the next time it opens the project |
...
cargo, gradle, maven, swift, dart, mix_build, vcpkg, cmake_build, dotnet_build and godot ship **disabled**
...
`enable_mix_build true` / `enable_vcpkg true` / `enable_cmake_build true` / `enable_dotnet_build true` / `enable_godot true`; while disabled
...
| `build_idle_days` | `45` | Extra idle gate for the build-tool adapters (cargo, gradle, maven, swift, dart, mix_build, vcpkg, cmake_build, dotnet_build, godot); applied as `max(build_idle_days, idle_days)` |
...
`enable_mix_build`, `enable_vcpkg`, `enable_cmake_build`, `enable_dotnet_build` and `enable_godot`, all of which make build

---
name: dev-prune
description: Use when asked to reclaim disk space, clean or prune dependency directories in any ecosystem (node_modules, .venv, target, vendor, Pods, _build, .terraform and the rest of its twenty-four package managers), restore project dependencies, size package manager caches, or diagnose the devp CLI. Covers the full dev-prune (devp) command surface, its safety rules, exit codes, and troubleshooting.
---
